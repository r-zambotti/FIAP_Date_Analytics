...
############################# Streamlit ############################
st.markdown(
    '<style>div[role="listbox"] ul{background-color: #6e42ad}; </style>', unsafe_allow_html=True)
...
input_grau_escolaridade = st.selectbox(
    'Qual o Grau de Escolaridade ?', dados['Grau_escolaridade'].unique())

# Estado civil
st.write('### Estado civil')
input_estado_civil = st.selectbox(
    'Qual é o seu estado civil ?', dados['Estado_civil'].unique())

# Número de membros da família
st.write('### Família')
membros_familia = float(
    st.slider('Selecione quantos membros tem na sua família', 1, 20))

# Carro próprio
st.write('### Carro próprio')
input_carro_proprio = st.radio(
    'Você possui um automóvel?', ['Sim', 'Não'], index=0)
...
input_casa_propria = st.radio(
    'Você possui uma propriedade?', ['Sim', 'Não'], index=0)
...
input_tipo_moradia = st.selectbox(
    'Qual é o seu tipo de moradia ?', dados['Moradia'].unique())

# Situação de emprego
st.write('### Categoria de renda')
input_categoria_renda = st.selectbox(
    'Qual é a sua categoria de renda ?', dados['Categoria_de_renda'].unique())

# Ocupação
st.write('### Ocupação')
input_ocupacao = st.selectbox(
    'Qual é a sua ocupação ?', dados['Ocupacao'].unique())

# Tempo de experiência
st.write('### Experiência')
input_tempo_experiencia = float(
    st.slider('Selecione o seu tempo de experiência em anos', 0, 30))

# Rendimentos
st.write('### Rendimentos')
input_rendimentos = float(st.text_input(
    'Digite o seu rendimento anual (em reais) e pressione ENTER para confirmar', 0))

# Telefone trabalho
st.write('### Telefone corporativo')
input_telefone_trabalho = st.radio(
    'Você tem um telefone corporativo?', ['Sim', 'Não'], index=0)
...
input_telefone = st.radio('Você tem um telefone fixo?', [
                          'Sim', 'Não'], index=0)
...
teste_novo_cliente = pd.concat(
    [teste_df, cliente_predict_df], ignore_index=True)
